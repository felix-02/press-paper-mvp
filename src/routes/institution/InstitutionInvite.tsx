...
export function InstitutionInvite() {
  ...
  const accept = async () => {
    ...
    if (result === "ok") {
      ...
      setError("Please confirm your email address first (check your inbox), then try again.");
  ...
  const Frame = ({ children }: { children: React.ReactNode }) => (
    <div style={{ minHeight: "100dvh", background: "var(--bg)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, display: "grid", placeItems: "center", padding: 24 }}>
        <div className="pp-card" style={{ width: "100%", maxWidth: 460, padding: 28, textAlign: "center" }}>{children}</div>
      </div>
    </div>
  );
